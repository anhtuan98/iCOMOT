...
/**
 *
 * This template describes two software units from different services which must
 * communicate, and thus the IP of one called unit must be send to the calling
 * unit.
 *
 * @author http://dsg.tuwien.ac.at
 */
public class DeployServiceWithCommunicatingUnitsBetweenMultipleServices {
    ...
    public static void main(String[] args) {
        ...
        //this time we just mention that the virtual container is allready deployd for
        //service with ID Service_1, and the container's ID in that service is VirtualContainer_1
        ServiceUnit virtualContainer_1 = OperatingSystemUnit("VirtualContainer_1")
                .andReference("Service_1/VirtualContainer_1");

        //the virtual container for the second unit must be deployed
        OperatingSystemUnit virtualContainer_2 = OperatingSystemUnit("VirtualContainer_1")
        ...
        //the software is allready deployed, so we just mention its reference, and that it exposes the IP
        ServiceUnit softwareUnit_1 = SingleSoftwareUnit("SoftwareUnit_1")
                .exposes(Capability.Variable("SoftwareUnit_1_Address"))
                .andReference("Service_1/SoftwareUnit_1");

        //rest of the file same as in DeployServiceWithCommunicatingUnits.java
